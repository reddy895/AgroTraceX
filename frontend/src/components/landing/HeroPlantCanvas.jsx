...
export const HeroPlantCanvas = () => {
  ...
  useEffect(() => {
    ...
    const fillLight = new THREE.DirectionalLight(0xaaaaaa, 1.4);
    ...
    const hoverGlowLight = new THREE.PointLight(0xffffff, 0.8, 8);
    ...
    const createLeaf = (length = 1.0, maxWidth = 0.46, archAngle = 0.5) => {
      ...
      const leafMesh = new THREE.Mesh(leafGeo, glassMat);
    ...
    // --- 4. Atmospheric Orbiting Particles ---
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      particleOrbitSpeeds[i] = (0.2 + Math.random() * 0.4) * (Math.random() > 0.5 ? 1 : -1);
      particleRadii[i] = radius;
      particleYOffsets[i] = Math.random() * Math.PI * 2;
    }

    particleGeo.setAttribute('position', new THREE.BufferAttribute(particlePos, 3));

    // Custom circle texture for soft glow motes
    ...
    const animate = () => {
      ...
      // Smooth lerping for plant rotation (gentle, never jerky)
      plantGroup.rotation.y = THREE.MathUtils.lerp(
        plantGroup.rotation.y,
        targetRotY + Math.sin(time * 0.6) * 0.06,
        0.04
      );
      plantGroup.rotation.x = THREE.MathUtils.lerp(
        plantGroup.rotation.x,
        targetRotX + Math.cos(time * 0.8) * 0.02,
        0.04
      );
      ...
      for (let i = 0; i < particleCount; i++) {
        const currentAngle = time * particleOrbitSpeeds[i] * 0.25 + i;
        const radius = particleRadii[i];
        positions[i * 3] = Math.cos(currentAngle) * radius;
        positions[i * 3 + 1] += Math.sin(time * 0.8 + particleYOffsets[i]) * 0.0015;
